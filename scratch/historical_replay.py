...
def replay_history(symbol="BTCUSDT", timeframes=["15m", "1h", "4h"], days=30):
    print(f"=== Historical Replay: {symbol} past {days} days ===")
    client = Client()
    ...
    for tf in timeframes:
        ...
        print(f"Fetching {days} days of {tf} for {symbol}...")
        ...
        # We must simulate incremental candle closing to avoid lookahead bias in indicators
        # But for speed, we can just compute indicators on the whole set (shift() handles lookahead)
        # However, to be perfectly safe, we'll do it incrementally or just trust shift()
        # features.py uses shift() and rolling() which is safe.
        df = add_features(df)
        ...
        for strat_name, strat_mod in strategies[tf]:
            ...
            for i in range(200, len(df)):
                ...
                # Only test BUYs due to LONG_ONLY
                if side == "SELL":
                    continue
