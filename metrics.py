...
def calculate_metrics(trade_history, equity_df, initial_balance):
    ...
    # For backtests on shorter timeframes, daily returns estimation requires resampled equity
    if not equity_df.empty:
        ...
        if len(daily_returns) > 1 and daily_returns.std() > 0:
            ...
            sortino = (daily_returns.mean() / downside.std()) * np.sqrt(365) if not downside.empty and downside.std() > 0 else 0.0
        else:
            sharpe = 0.0
            sortino = 0.0
            
        calmar = (daily_returns.mean() * 365) / (max_dd_pct / 100) if max_dd_pct > 0 else 0.0
    else:
        sharpe = 0.0
        sortino = 0.0
        calmar = 0.0
