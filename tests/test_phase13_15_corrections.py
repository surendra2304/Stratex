...
class TestClassificationGate:
    ...
    def _make_cfg(self, min_trades=30):
        return FrozenExperimentConfig(
            experiment_name="gate_test",
            planned_duration_days=30, # kept for backward compat of config
            min_required_trades=min_trades,
        )

    def test_trades_alone_trigger_classification(self):
        from paper_engine.statistical_report import can_classify
        cfg = self._make_cfg()
        result = can_classify(cfg, n_closed_trades=35)
        assert result["allowed"] is True
        assert result["verdict"] == "ALLOWED"

    def test_insufficient_trades_blocks_classification(self):
        from paper_engine.statistical_report import can_classify
        cfg = self._make_cfg()
        result = can_classify(cfg, n_closed_trades=5)
        assert result["allowed"] is False
        assert result["verdict"] == "BLOCKED_TRADES"

    def test_can_classify_required_keys(self):
        from paper_engine.statistical_report import can_classify
        cfg = self._make_cfg()
        result = can_classify(cfg, 35)
        required_keys = ["allowed", "verdict", "message", "trade_gate", "n_closed_trades"]
        for key in required_keys:
